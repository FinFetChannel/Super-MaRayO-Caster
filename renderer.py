...
def rayCaster(player, mapa, frame, horizontal_res, vertical_res, mod, textures, step, GetTile, even, progresive):
    ...
    sky_x = -0.5*vertical_res +player.roth*vertical_res
    sky_y = -2*horizontal_res +player.rot*horizontal_res
    
    if player.bonus:
        sub_sky = pg.Surface.subsurface(textures[0], (-sky_x, -sky_y + vertical_res - 0 -1, horizontal_res, 1)).copy()
        sub_sky.fill((0,64,64))
        
    for i in range(vertical_res): # Iterate over each point on the frame
        if progresive or i%2 == even:
            if not player.bonus:
                sub_sky = pg.Surface.subsurface(textures[0], (-sky_x, -sky_y + vertical_res - i -1, horizontal_res, 1))
            frame.blit(sub_sky, (0, vertical_res - i -1))
            ...
            for ray in range(int(3/step)): # Coarse ray loop
            ...
            for ray in range(int(1/step)): # Fine ray loop
